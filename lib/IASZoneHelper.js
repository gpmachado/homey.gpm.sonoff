...
class IASZoneHelper {
  ...
  // Alarm 1 only: on sensors that use a single alarm the alarm 2 bit can be stuck or
  // random (zigbee-herdsman-converters iasZoneAlarm), so it must not count as an alarm.
  static hasAlarm1(value) {
    return !!(IASZoneHelper.toUint16(value) & IAS_BITS.alarm1);
  }

  static hasBatteryLow(value) {
    return !!(IASZoneHelper.toUint16(value) & IAS_BITS.battery);
  }
