 ...
 * @description Zigbee device availability tracking.
...
// Outbound failures ("Could not reach device") only count as evidence when there are
// several of them: N failures at least MIN_GAP apart (so one burst of retries for a
// single command is one failure), with no inbound frame from the device in between.
// Only then a confirmation poll runs, at most once per COOLDOWN.
const SEND_FAILURES_TO_CONFIRM = 3;
// Longer than a whole SonoffBase.readAttribute retry chain (failures land up to ~60 s
// apart), so one logical read of an unreachable device counts once.
const SEND_FAILURE_MIN_GAP_MS = 60 * 1000;
const SEND_FAILURE_COOLDOWN_MS = 60 * 1000;
// A hub-side outage (Homey radio down) fails every send too, so outbound failures
// only count once the device has also been silent for this long (capped at half the
// device's own timeout).
const SEND_FAILURE_MIN_SILENCE_MS = 10 * 60 * 1000;
// Store key holding the reason while unavailable: ZigBeeDevice.onNodeInit calls
// setAvailable() on every init, so the state is re-applied at install.
const UNAVAILABLE_STORE_KEY = 'availability_unavailable_reason';
...
class AvailabilityManagerBase {
  ...
  constructor(device, options = {}) {
    ...
    this._sendFailuresToConfirm = options.sendFailuresToConfirm ?? SEND_FAILURES_TO_CONFIRM;
    this._sendFailureMinGapMs = options.sendFailureMinGapMs ?? SEND_FAILURE_MIN_GAP_MS;
  ...
  async _persistMessageStats(force = false) {
    ...
    try {
      ...
      // The device is already deleted when uninstall() runs from onDeleted().
  ...
  async _markAlive(source) {
    try {
      const now = Date.now();
      const gapMin = this._lastSeen ? Math.round((now - this._lastSeen) / 60000) : 0;
      this._lastSeen = now; // in-memory truth, updated unconditionally
      this._activitySeen = true;
      if (this.options.logIdle && gapMin >= 3) {
        this.device.log(`[Availability] Activity after ${gapMin}min of silence (${source})`);
      }
      this._sendFailures = 0; // the device answered: earlier outbound failures were transient
  ...
  /**
   * Polls the device before marking it offline.
   * Reads an attribute of the Basic cluster (0x0000), which every Zigbee device has.
   * @returns {Promise<boolean|null>} true if the device answered, false if it did not,
   *   null if a poll is already in progress (evidence of nothing - the caller skips)
   */
  async _pollDevice() {
  ...
  _startWatchdog() {
    ...
    // Random phase within one interval: otherwise every watchdog started at boot
    // ticks on the same second, and their store writes and logs all land at once.
  ...
  /**
   * Mark all sibling devices available and fire the availability_turned_on flow card.
   */
  async _markAllAvailable() {
    ...
    await this.device.setStoreValue(UNAVAILABLE_STORE_KEY, '').catch(() => {});
  ...
  async _markAllUnavailable(reason) {
    ...
    await this.device.setStoreValue(UNAVAILABLE_STORE_KEY, reason).catch(() => {});
  ...
  /**
   * ZigBeeDevice.onNodeInit marks the device available on every init, so a device
   * that was unavailable before an app restart would look available for the whole
   * boot grace. Re-apply the persisted state instead.
   */
  async _reapplyUnavailable() {
    const reason = this.device.getStoreValue(UNAVAILABLE_STORE_KEY);
    // A frame that already arrived during install is proof of life - keep it available.
    if (!reason || !this.device.getAvailable() || this._activitySeen) return;
    this.device.log(`[Availability] Unavailable: ${reason} (restored after restart)`);
    await this.device.setUnavailable(reason).catch(() => {});
  }
   ...
   * the sibling cascade and
   * onBecameAvailable() callback both fire correctly.
   */
  async markAvailable() {
   ...
   * that the sibling cascade and
   * onBecameUnavailable() callback both fire correctly.
   * @param {string} reason
   */
...
class AvailabilityManagerPassive extends AvailabilityManagerBase {
  ...
  async install() {
    ...
    try {
      ...
      await this._reapplyUnavailable();
  ...
  _onSendFailure(err) {
    if (!this._installed || this._polling || this._confirmingFailure) return;
    // Homey's send error carries no code, only this text (see wrapper note above).
    if (!/could not reach device/i.test(err?.message || '')) return;
    if (!this.device.getAvailable()) return;
    const now = Date.now();
    if (now - this._installedAt < this._bootGraceMs) return;
    // Homey-side outage guard: a silent device, not just failing sends.
    const minSilence = Math.min(this.options.timeout / 2, SEND_FAILURE_MIN_SILENCE_MS);
    if (now - this._lastSeen < minSilence) return;
    if (now - (this._lastCountedFailureAt ?? 0) < this._sendFailureMinGapMs) return; // same burst
    this._lastCountedFailureAt = now;

    this._sendFailures += 1;
    if (this._sendFailures < this._sendFailuresToConfirm) {
      this.device.log(`[Availability] Send failed (${this._sendFailures}/${this._sendFailuresToConfirm}): ${err.message}`);
      return;
    }
    if (now - (this._lastFailureCheckAt ?? 0) < SEND_FAILURE_COOLDOWN_MS) return;

    this._lastFailureCheckAt = now;
    this._confirmingFailure = true;
    this.device.log(`[Availability] Send failed ${this._sendFailures}x in a row - confirming with a poll...`);
    this._pollDevice()
      .then(alive => (alive === false ? this._markAllUnavailable('Command failed: device unreachable') : null))
      .catch(e => this.device.error('[Availability] Send-failure check error:', e.message))
      .finally(() => { this._confirmingFailure = false; });
  }
