...
//
// Accepted writes are logged too ("Write OK ..."), independent of ZCL_DEBUG,
// so a setting change is confirmed in the log without raw frame dumps. Per
// ZCL, an all-success response is a single SUCCESS record without attribute
// id - so "accepted" is every requested attribute that wasn't rejected.
async function writeAttributesVerbose(device, cluster, attributes) {
  const response = await cluster.writeAttributes(attributes);
  const endpointId = cluster._endpoint?._endpointId;
  const where = `${cluster.constructor.NAME}${typeof endpointId === 'number' ? ` ep${endpointId}` : ''}`;
  ...
  if (response && Array.isArray(response.attributes)) {
    for (const entry of response.attributes) {
      if (entry.status !== 'SUCCESS') {
        ...
        device.error(
          `Device REJECTED write to '${attrName}' on ${where}: ${entry.status}`,
        );
      }
    }
  }

  const accepted = Object.entries(attributes)
    .filter(([name]) => !rejectedIds.has(cluster.constructor.attributes?.[name]?.id))
    .map(([name, value]) => `${name}=${value}`);
  if (accepted.length) device.log(`Write OK on ${where}: ${accepted.join(', ')}`);
 ...
 * instance, prefixed with the device's own name (e.g. "[Office
 * Power Supply]"), instead of relying on the SDK's [Device:<uuid>] prefix.
 *
